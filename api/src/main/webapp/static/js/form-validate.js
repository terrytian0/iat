...
$.validator.addMethod("checkName",function(value,element,params){
    var checkName = /^\w{2,20}$/g;
    return this.optional(element)||(checkName.test(value));
},"*只允许2-20位英文字母、数字或者下画线！");

$.validator.addMethod("checkEmail",function(value,element,params){
    var checkEmail = /^([a-z0-9-_\.]+@([a-z0-9-]+\.)+[a-z]{2,4}\,)*([a-z0-9-_\.]+@([a-z0-9-]+\.)+[a-z]{2,4})$/i;
    return this.optional(element)||(checkEmail.test(value));
},"*请输入正确的邮箱！");
$.validator.addMethod("checkEnglishName",function(value,element,params){
    var checkEnglishName = /^[a-z\d]{1,10}[.]{0,1}[a-z\d]{0,10}$/;
    var checkEmailName = /^[a-z\d]{1,10}[.]{0,1}[a-z\d]{0,10}@xx\.com$/;
    return this.optional(element)||(checkEnglishName.test(value)) || checkEmailName.test(value);
},"*请输入正确的用户名，例如:san.zhang 或san.zhang@xx.com");

$.validator.addMethod("checkDomain",function(value,element,params){
    var checkDomain = /^[a-zA-Z\d][-a-zA-Z\d]{0,62}(\.[a-zA-Z\d][-a-zA-Z\d]{0,62})+$/i;
    return this.optional(element)||(checkDomain.test(value));
},"*请输入正确的域名！");
$.validator.addMethod("checkUrl",function(value,element,params){
    var checkUrl = /^((ht|f)tps?):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:\/~\+#]*[\w\-\@?^=%&\/~\+#])?$/i;
    return this.optional(element)||(checkUrl.test(value));
},"*请输入正确的Url，例如:http://git.xx.com/xx.git");

$.validator.addMethod("checkDepend",function(value,element,params){
    var checkUrl = /^(([\w-\.\/]+)jar\,)*(([\w-\.\/]+)jar)$/;
    return this.optional(element)||(checkUrl.test(value));
